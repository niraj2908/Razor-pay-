...
| 3 | `/overview` | Populated operating console | Revenue at risk, recovery opportunity, decision mix, recent activity | "Synthetic and labelled as such on the page. ₹1,17,598.25 at risk, ₹44,106.99 of expected incremental value, 54 Act / 1 Wait / 2 Stop / 2 Escalate. The system is willing to *not* act." | **Synthetic** | 0:35 |
| 4 | `/recovery` | Operator work queue | Diagnosis, amount at risk, decision badge per row | "The two biggest amounts here are escalations, not actions — low confidence on a big payment goes to a human." | **Synthetic** | 0:25 |
...
| 6 | `/security` | **Real Razorpay evidence — credibility anchor** | The "Live end-to-end lifecycle · Verified" row (branches: ACT, STOP); narrate the real chain | "A real Test Mode payment failed at authorization, the webhook was signature-verified, the engine chose Act, an operator approved it, and Razorpay created a real Payment Link." | **Real (Test Mode)** | 0:45 |
...
| 9 | `/experiments/demoexperimentpaymentlinknudge` | Observed vs causal | Treatment 81.8% (22 units) vs control 17.9% (28 units), 95% CI 37–79 pp, VALID_EFFECT, causal incremental ₹20,037.99 | "Without the control arm we'd have called natural recovery a win. This is synthetic demo data." | **Synthetic** | 0:20 |
